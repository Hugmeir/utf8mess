...
This new feature is still considered experimental.  To enable it, use:

    use 5.018;
    no warnings "experimental:infix_subs";
    use feature "infix_subs";

This enables the use of the "C<< > >>" prototype, which allows you to declare
subs like C<sub foo ($>$) { ... }>.  These functions are then usable in
a similar fashion as the C<eq> or C<ne> operators:

    sub length_eq ($>$) { length($_[0]) eq length($_[1]) }
    say "foo" length_eq "bar" ? "Same length" : "Different length";

See L<perlsub/Infix Subroutines> for more information.
